...
import { Alert, AlertActionCloseButton, AlertGroup, Button, ThemeToggle } from '../../src/ui';
import { BarsIcon } from '../../src/ui/icons';
import { InventorySidebar, ContextTrail, routeTitle } from './app-shell';
import { WorkspacesPage, WorkspaceCreateDialog, EmptyWorkspaces } from './workspace-pages';
import { ProductsPage, ProductComponentsPage, ProductCreatePage, ProductPage, ProductArchitecturePage } from './product-pages';
...
export function InventoryApp({ route, navigate }: { route: AtlasRoute; navigate: Navigate }) {
  ...
  useEffect(() => {
    if (navigationOpen) document.querySelector<HTMLButtonElement>('#application-sidebar button')?.focus();
  }, [navigationOpen]);
  ...
  const go = (next: AtlasRoute) => { setNavigationOpen(false); navigate(next); };
  ...
  return <div className="alpa-app inventory-app" data-navigation-open={navigationOpen}>
    ...
    <div className="app-frame">
      <header className="topbar">
        <Button className="navigation-toggle" aria-label="Toggle navigation" aria-expanded={navigationOpen} aria-controls="application-sidebar" onClick={() => setNavigationOpen(!navigationOpen)} icon={<BarsIcon />} />
      ...
      <main id="main-content" className="content" tabIndex={-1}><div className="surface-transition">{page}</div></main>
    </div>
    ...
    {toast ? <AlertGroup isToast isLiveRegion><Alert role="status" variant="success" title={toast} actionClose={<AlertActionCloseButton onClose={() => setToast('')} />} /></AlertGroup> : null}
  </div>;
}
...
export function renderInventoryRoute({
  ...
  if ('productKey' in route && inventory.productsStatus === 'error') return <RequestError message={inventory.productsError} />;
  switch (route.kind) {
    case 'overview':
    ...
    case 'component-create':
      ...
      return <ComponentCreatePage product={product} onClose={() => navigate({ kind: 'product', productKey: product.product_code })} onCreate={async (input) => {
    ...
    case 'product':
      if (!product) return inventory.productsStatus === 'loading' ? <LoadingState label="Loading product" /> : <NotFound label="Product not found" onBack={() => navigate({ kind: 'products' })} />;
      if (route.tab === 'components') return <ProductComponentsPage product={product} components={inventory.components} status={inventory.componentsStatus} error={inventory.componentsError} navigate={navigate} />;
    ...
    case 'component': {
      if (!product) return inventory.productsStatus === 'loading' ? <LoadingState label="Loading product" /> : <NotFound label="Product not found" onBack={() => navigate({ kind: 'products' })} />;
      if (inventory.componentsStatus === 'error') return <RequestError message={inventory.componentsError} />;
      ...
      return <ComponentPage product={product} component={component} components={inventory.components} navigate={navigate} addConsumerAPI={inventory.addConsumerAPI} createComponentClient={inventory.createComponentClient} refreshComponent={inventory.refreshComponent} notify={notify} />;
    }
...
export { buildComponentInput, relationshipCandidates } from './component-input';
export type { ComponentDraft, RelationshipCandidate } from './component-input';
