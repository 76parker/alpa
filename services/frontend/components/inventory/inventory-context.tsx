  ...
  switch (action.type) {
    ...
    case "workspaces_failed":
      return {
        ...
        workspacesError: action.message,
      };
    ...
    case "product_selected":
      return {
        ...
        components: [],
        componentsStatus:
          action.productID === null ? "idle" : state.componentsStatus,
        componentsError: "",
      };
    ...
    case "components_failed":
      ...
      return {
        ...
        componentsError: action.message,
      };
    ...
    case "component_refreshed":
      ...
      return {
        ...
        components: state.components.map((item) =>
          item.id === action.component.id ? action.component : item,
        ),
      };
  }
}
...
export function InventoryProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    inventoryClient
      .listAllWorkspaces(controller.signal)
      .then((workspaces) => {
  ...
  const loadProducts = useCallback(
    async (workspaceID: number, signal?: AbortSignal) => {
      dispatch({ type: "products_loading", workspaceID });
      try {
        const products = await inventoryClient.listAllProducts(
          workspaceID,
          signal,
        );
  ...
  const createProduct = useCallback(
    async (input: CreateProductInput) => {
      ...
      const product = await inventoryClient.createProduct(
  ...
  const clearProductSelection = useCallback(
    () => dispatch({ type: "product_selected", productID: null }),
    [],
  );
  ...
  const createComponentAPI = useCallback(
    async (componentID: number, input: CreateAPIInput) => {
      await inventoryClient.createComponentAPI(componentID, input);
      return refreshComponent(componentID);
    },
    [refreshComponent],
  );

  const createComponentClient = useCallback(
    async (componentID: number, input: CreateClientInput) => {
  ...
  const bindComponentClient = useCallback(
    async (componentID: number, clientID: number, apiID: number) => {
      await inventoryClient.bindComponentClient(componentID, clientID, {
        api_id: apiID,
      });
      return refreshComponent(componentID);
    },
