...
export function makeComponentRequest(
  ...
  if (value.type === "infrastructure") {
    ...
    return {
      ...
      apis: value.apis.map((api) => ({ ...api, api_type: system.apiType })),
    };
  }
  return {
    product_id: productID,
    ...
    details: { language: value.language },
    apis: value.apis,
...
export function makeClientRequest(
  ...
  return {
    ...value,
    ...(original
      ? { action: original.action, capabilities: original.capabilities }
      : {}),
  };
}
export function makeAPIRequest(
  draft: z.input<typeof apiSchema>,
  documentationURL?: string | null,
): APIInput {
  return {
    ...apiSchema.parse(draft),
